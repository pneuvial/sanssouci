...
#'
#' @examples
res_all_stat <- function(Result_hmm_boot, seuil, min_size){
...
#' @examples
#' 
res_FDR <- function(Result_hmm_boot, seuil, min_size ){
  alpha <- Result_hmm_boot$al %>% unique()
  var1 <- "'{'"
  var2 <- "'}'"
  Res_all <-   Result_hmm_boot %>% 
    ...
    mutate(
      ...
      FDR_boot_naif = map_dbl(Est_HMM_boot, ~ pull(. , Real_boot) %>% 
                                mean()),
      V_simes = V_simes / Size, 
      V_simes_small = V_simes_small / Size,
      FDR_boot_naif = FDR_boot_naif / Size, 
      FDR_est = FDR_est/Size,
      Nom = case_when( Nom == "pval_tresh" ~  glue("S(X) == bgroup({var1},p <", seuil,",{var2})"), 
                       Nom == "sel_viter_est" ~ "S(X) == bgroup('{',viterbi == 1,'}')",
                       Nom == "sel_viter_or" ~ "S(X) == bgroup('{',viterbi[or] == 1,'}')",
                       Nom == "sel_viter_min_size" ~ glue("S(X) == bgroup({var1},'viterbi = 1 & size >", min_size,"',{var2})"),
                       Nom == "H0_and_pval_tresh_small" ~  glue("S(X) ==H0~intersect()~ bgroup('{',p <", seuil," ,'}')"), 
                       Nom == "H0_and_sel_viter_est" ~ "S(X) ==H0~intersect()~bgroup('{',viterbi == 1 ,'}')",
                       Nom == "H1" ~  "S(X) == H1", 
                       Nom == "pval_ord" ~  "S(X) == bgroup('{',p < th,'}')",
      ...
      Nom = factor(Nom, levels = c(glue("S(X) == bgroup({var1},p <", seuil,",{var2})"),
                                   ...
                                   "S(X) == bgroup('{',viterbi[or] == 1,'}')",
                                   "S(X) == SC(FDR[v])",
                                   glue("S(X) == bgroup({var1},'viterbi = 1 & size >", min_size,"',{var2})"), 
                                   "S(X) ==bgroup('{','1, ..., 200' ,'}')",
                                   "S(X) == H1",
                                   "S(X) ==H0~intersect()~ bgroup('{',p < 0.05 ,'}')", 
                                   "S(X) ==H0~intersect()~bgroup('{',viterbi == 1 ,'}')"))
